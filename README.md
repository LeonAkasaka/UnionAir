...
| **Build** | Build configuration, build scenes, scripting backend and define symbols, installed platform modules | Disabled by default |
...
- The **Build** category carries the `executableOutput` risk: it is the category through which UnionAir reports build configuration and, in later releases, produces player builds. Build configuration includes scripting define symbols and product identity, which describe how the project is compiled and shipped. Its endpoints today are read-only, but the category is disabled by default because of what it is the gate for.
