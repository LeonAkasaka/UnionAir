namespace LeonAkasaka.UnionAir.Editor
{
    ...
    internal sealed class AnimatorControllersController
    {
        ...
        [UnionAirEndpoint("POST", "{guid}/states",
            Category = UnionAirEndpointCategories.AssetWrite,
            PlayModePolicy = UnionAirPlayModePolicy.Blocked,
            Summary = "Adds a state to a layer of an AnimatorController, fully formed: every setting PATCH accepts may be supplied here. 'motion' is an object with a 'guid' referencing an AnimationClip. 'position' is {x,y} graph layout, so a controller authored through the API does not stack every state at the origin. A '*Parameter' naming a parameter the controller does not have is a 400, and every value is checked before the state is created, so a rejected request adds nothing. 'behaviours' is read-only and reported in 'unsupported'. An unknown field is a 400 listing the accepted ones.",
            PathParams = new string[] { "guid" },
            RequiredBody = new string[] { "name" },
            OptionalBody = new string[] { "layerIndex", "setAsDefault", "motion", "speed", "tag", "writeDefaultValues", "iKOnFeet", "mirror", "cycleOffset", "speedParameter", "speedParameterActive", "cycleOffsetParameter", "cycleOffsetParameterActive", "mirrorParameter", "mirrorParameterActive", "timeParameter", "timeParameterActive", "position" },
        ...
        [UnionAirEndpoint("PATCH", "{guid}/states",
            Category = UnionAirEndpointCategories.AssetWrite,
            PlayModePolicy = UnionAirPlayModePolicy.Blocked,
            Summary = "Updates a state in an AnimatorController, addressed by 'name' and optionally 'layerIndex'. Every other field is optional and an omitted field is left unchanged. A '*Parameter' and its '*Active' flag are one decision: a name the controller does not have is a 400 and neither half is written. Every value is checked before the first is written, so a rejected field leaves the state as it was. 'behaviours' is read-only and reported in 'unsupported'. An unknown field is a 400 listing the accepted ones.",
            PathParams = new string[] { "guid" },
            RequiredBody = new string[] { "name" },
            OptionalBody = new string[] { "layerIndex", "newName", "setAsDefault", "motion", "speed", "tag", "writeDefaultValues", "iKOnFeet", "mirror", "cycleOffset", "speedParameter", "speedParameterActive", "cycleOffsetParameter", "cycleOffsetParameterActive", "mirrorParameter", "mirrorParameterActive", "timeParameter", "timeParameterActive", "position" },
