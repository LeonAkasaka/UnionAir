namespace LeonAkasaka.UnionAir.Editor
{
    ...
    internal sealed class ShadersController
    {
        [UnionAirEndpoint("GET", "{guid}",
            Category = UnionAirEndpointCategories.Read,
            Summary = "Returns a shader's import state, cached compiler messages, its effective local keyword space, declared properties with their defaults, and the subshaders Unity compiled. 'hasError' and 'messages' are the diagnostics Unity currently has cached for the shader; once a reimport has settled they answer whether Unity accepted that import, which reading the .shader file cannot. Check 'isUpdating' if you need to be sure it has. Messages are the set Unity currently has cached, which a reimport clears and refills, so reimport the asset before reading again. The set also grows on its own, because Unity compiles variants on demand: a clean 'hasError' means nothing has compiled a broken variant yet rather than that the shader is valid, and an error appearing in a later read with no edit in between is not a fault. 'isSupported' is Unity's capability signal — whether the shader runs on the current GPU, fallbacks considered — and says nothing about whether the import succeeded. 'keywords' is the effective space and includes keywords from Fallback/UsePass dependencies and keywords Unity adds, not only the shader's own declarations. 'subshaders' is what Unity compiled, which is the Fallback's when the shader's own subshaders are unusable. Each subshader reports its 'renderPipeline' tag, which says which pipeline that subshader is for; it is null when the subshader declares no such tag, as a built-in-pipeline subshader does. Every structural field is null only when the ShaderLab parse failed before the shader's name was read, because nothing then came from the file. A .shadergraph is read the same way and needs no separate endpoint: Unity's importer generates a shader and makes it the asset's main object, so properties, keywords, passes and the generated shader name are all reported. 'hasImportError' and 'importMessages' are the asset importer's log, which is a different set from the compiler's 'messages': the compiler only sees the shader Unity ended up with, and a Shader Graph that failed is replaced by a substitute that compiles cleanly, so 'hasError' cannot report it and only the importer's log can. It is not exhaustive, and a clean 'hasImportError' is not proof of a clean import: measured on 6000.0.80f1, a graph that parses and cannot be built, and a target that does not resolve, reach neither log — see the API reference for the boundary. Both are null when the asset has no importer to ask, as a shader Unity ships does. A guid whose asset produced no shader answers 400 and carries the importer's log with it; it reads 'Shader asset failed to import' only when the import failed and the extension is .shader or .shadergraph, because every importer writes to that same log.",
            PathParams = new string[] { "guid" },
            ResponseExample = "{\"guid\":\"5ebb6c...\",\"assetPath\":\"Assets/Shaders/Toon.shader\",\"name\":\"Toon/Toon\",\"isSupported\":true,\"hasError\":false,\"hasWarnings\":false,\"messages\":[],\"hasImportError\":false,\"hasImportWarnings\":false,\"importMessages\":[],\"renderQueue\":2000,\"maximumLOD\":-1,\"subshaderCount\":1,\"passCount\":2,\"keywords\":[{\"name\":\"_ALPHATEST_ON\",\"isOverridable\":false,\"isDynamic\":false}],\"properties\":[{\"name\":\"_BaseColor\",\"type\":\"Color\",\"description\":\"Base Color\",\"defaultValue\":{\"r\":1,\"g\":1,\"b\":1,\"a\":1},\"flags\":[\"MainColor\"],\"attributes\":[]},{\"name\":\"_AlphaClip\",\"type\":\"Float\",\"description\":\"Alpha Clipping\",\"defaultValue\":0,\"flags\":[],\"attributes\":[\"Toggle(_ALPHATEST_ON)\"]},{\"name\":\"_MainTex\",\"type\":\"Texture\",\"description\":\"Base Map\",\"defaultValue\":\"white\",\"textureDimension\":\"Tex2D\",\"flags\":[\"MainTexture\"],\"attributes\":[]}],\"activeSubshaderIndex\":0,\"subshaders\":[{\"levelOfDetail\":300,\"renderPipeline\":\"UniversalPipeline\",\"passes\":[{\"name\":\"ForwardLit\",\"lightMode\":\"UniversalForward\",\"isGrabPass\":false}]}]}")]
        private void Get(UnionAirRequestContext ctx)
