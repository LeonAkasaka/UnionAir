namespace LeonAkasaka.UnionAir.Editor
{
    ...
    internal sealed class AnimatorControllersController
    {
        ...
        [UnionAirEndpoint("GET", "{guid}",
            Category = UnionAirEndpointCategories.Read,
            Summary = "Returns the full AnimatorController structure: layers, states, transitions, and parameters. Every 'motion' carries a 'type' of AnimationClip, BlendTree, or Unknown. A blend tree is a sub-asset of the controller, so its 'guid' is null and its structure is serialized inline, nested trees included. A clip reports 'assetPath' and 'clipsAtPath'; a clip imported from a model file shares its GUID with the file, so 'clipsAtPath' above 1 means the GUID does not identify one clip. Sub-state machines are not enumerated.",
            PathParams = new string[] { "guid" },
