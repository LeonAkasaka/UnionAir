...
- 既定で有効なのは **Read** カテゴリのみです。Scene Write / Asset Write / Play Mode / Editor Actions はオプトインです。有効化すると、Unity Editor のメニュー実行やアセット削除を含む状態変更操作が、任意のローカルクライアント・ブラウザオリジンに公開されます。すべてのローカルクライアントを信頼できる場合にのみ有効化してください。
