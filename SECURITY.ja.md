...
- 既定で有効なのは **Read** カテゴリのみです。Scene Write / Asset Write / Play Mode / Editor Actions / Test Runner / Profiling はオプトインです。有効化すると、プロジェクトの任意のテストコード、Unity Editor のメニュー実行、アセット削除、ダウンロード可能な heap snapshot を含む操作と診断成果物が、任意のローカルプロセスに公開されます。すべてのローカルクライアントを信頼できる場合にのみ有効化してください。
