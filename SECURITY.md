...
- A committed `.unionair/settings.json` can request capabilities but cannot grant them. Each user approves sensitive categories, custom handlers, and Play Mode scene changes separately for the normalized project path. Effective access is the project request intersected with local approvals, minus local disables. Invalid project settings fail closed: auto-start is disabled and only Read remains available.
- When `.unionair/settings.json` is absent, category enablement retains the legacy `EditorPrefs` behavior and is shared by projects opened by the same user and Editor version. Local project approvals are also stored in `EditorPrefs`, but are keyed by normalized project path and are never committed.
- Enabling a sensitive capability in the EditorWindow both adds the project request and approves it locally on that machine. The shared file still grants nothing elsewhere. Schema-backed UI changes are saved to the project file immediately; external edits are not reread until the Editor process restarts.
