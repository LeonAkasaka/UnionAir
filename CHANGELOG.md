...
- Added strict versioned project configuration through `.unionair/settings.json`. The EditorWindow can save and reload the complete v1 document, shows its validity and source, and manages project-scoped local approval, refusal, revocation, and local disables for sensitive capabilities. The file is UTF-8 without a BOM, written atomically, and remains visible to Git while runtime discovery files stay ignored.
