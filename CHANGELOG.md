...
- Added strict versioned project configuration through `.unionair/settings.json`. Schema-backed EditorWindow controls update an in-memory working document and atomically save the complete UTF-8-without-BOM v1 file after every change; failed writes remain dirty and retry automatically. Domain reloads restore the working document through SessionState instead of rereading external edits. The window shows source, validity, and persistence state, and manages project-scoped local approval, refusal, revocation, and local disables for sensitive capabilities. The file remains visible to Git while runtime discovery files stay ignored.
...
- Project settings no longer make their EditorWindow controls read-only. Port mode is locked only while the listener is running; server, category, custom-handler, and Play Mode safety settings remain editable and save automatically without manual Save or Reload actions. Enabling a sensitive setting in the UI records the project request and its local approval together, while Local Access can still deny an approved request without rewriting the file.
