...
マテリアルとそのシェーダーが食い違っている箇所を報告します。シェーダーがもう宣言していないプロパティに対してマテリアルが保持し続けている値、シェーダーが宣言しているのにマテリアルが値を持っていないプロパティ、そしてマテリアルが有効にしているがシェーダーに居場所のないキーワードの3つです。
...
| `staleProperties[]` | シェーダーがもう宣言していないプロパティに対して、マテリアルが保持し続けている値。Unity は `.mat` ファイル内に保持したまま隠すため、他のどのエンドポイントもこれを報告しません — [stale プロパティとは何か](#stale-プロパティとは何か) を参照 |
...
[`GET /api/assets/materials/{guid}`](#get-apiassetsmaterialsguid) はシェーダーが現在宣言しているプロパティとマテリアルの値を返し、[`GET /api/assets/shaders/{guid}`](#get-apiassetsshadersguid) はシェーダーの宣言内容を返します。どちらもシェーダーの宣言をたどるため、シェーダーが落としたプロパティに対してマテリアルが持ち続けている値は、そのどちらにも現れません。

Unity はその値を捨てません — `.mat` ファイルに残したまま Inspector から隠します。そのためシェーダーのプロパティ名を変更すると、古い値は行方の説明もないまま単に消えたように見えます。`staleProperties` はそれに答えるもので、名前だけでなく値も返します。「何かが失われる」ではなく「失われるのはこれだ」が答えになるようにするためです。
