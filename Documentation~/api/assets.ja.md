...
`hasError` と `messages` は**シェーダーコンパイラ**のものです。`hasImportError` と `importMessages` は**アセットインポーター**のものです。統合せずに並置しているのは、シェーダーが片方だけで失敗しうるからであり、生成シェーダーにとっては後者だけが語れる場面があるからです。

インポーターがビルドできなかった Shader Graph は、Shader Graph 自身のエラーシェーダーに置き換えられ、そのグラフが本来名乗るはずだった名前が付けられます。この代替シェーダーはクリーンにコンパイルされるため、コンパイラには報告することがなく `hasError` は `false` になります。インポーターのログがなければ、プロパティが少ないだけのシェーダーとレスポンス上で区別できません。
...
**実測で判明した限界。** Shader Graph の失敗すべてがインポートログに届くわけではありません。6000.0.80f1 / Shader Graph 17.0.4 での実測:

| 失敗の種類 | エンドポイントの報告 |
| --- | --- |
| ファイルがパースできない | `400`、`hasImportError` が `true`、`importMessages` にインポーターの例外 |
| グラフはビルドされるがノード/コンパイラエラーがある | 報告される(Shader Graph がインポートコンテキスト経由で記録するため) |
| グラフはパースできるがビルドできない | **何も出ない** — `hasError` は `false`、`importMessages` は `[]`、無名パス1件、プロパティなし |
| ターゲットが解決できない | どちらのログにも**何も出ない**。サブシェーダーは黙って落ち、Shader Graph はインポートコンテキストではなく `Debug.LogError` で Console に書きます |

後ろの2件については [`GET /api/editor/logs`](editor.ja.md#get-apieditorlogs) にメッセージがあります(アセットパスを前置した散文として)。確実を期すクライアントは、`hasError` がクリーンであることを信じるのではなく、`subshaders[].renderPipeline` を期待したパイプラインと突き合わせてください。
