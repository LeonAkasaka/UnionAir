...
| `hasError`, `hasWarnings` | Unity がこのシェーダーについて現在キャッシュしている診断にエラー/警告が含まれるかどうか。再インポート直後に読めばそのインポートの判定になりますが、Unity がバリアントをコンパイルするたびに集合は増えます — [`hasError` は Unity がこれまでにコンパイルしたバリアントの話](#haserror-は-unity-がこれまでにコンパイルしたバリアントの話) を参照。`hasError` は「そのシェーダーが使用不能」を意味**しません**。`isSupported` を参照してください |
...
- **Unity がそれを受け入れたかどうか。** シェーダーのコンパイルはインポート時に行われ、失敗したシェーダーも見た目はそのままディスク上に残ります。再インポート後に読めば、`hasError` と `messages` は [`POST /api/compile`](compile.ja.md) が C# に対して閉じているのと同じ「編集 → インポート → 診断」のループをシェーダーに対して閉じます。
