...
この 2 コールがループのすべてで、その横にシェーダー専用の検証エンドポイントはありません。再インポートは応答前に完了し、アセット 1 件を対象とし(Unity がまだ知らないファイルも含む)、プロジェクト内の `#include` は include 側・シェーダー側のどちらを再インポートしても届きます。できないのは、**あなたがこのループを飛ばしたことに気づくこと**です。ディスク上で編集したシェーダーを再インポートせずに read すると前回のインポートのメッセージが返りますが、Unity は「ディスクと相違がある」というシグナルを公開しておらず、read が古いと示す手段がありません。
...
### `hasError` がクリーンでも、それはインポートがコンパイルしたバリアントの話

インポートはそのシェーダーが生成しうるすべてのバリアントをビルドするわけではなく、`messages` は一度もビルドされなかったバリアントのエラーを運べません。

6000.0.80f1 で、`#pragma multi_compile` のキーワード側の分岐だけが壊れているシェーダーを実測しました。再インポート後も read は `hasError` が `false`、`messages` が `[]` と答えます。そのキーワード下ではコンパイルが通らないのに、エンドポイントは何も言いません。壊れている分岐が、インポートのビルドしなかったバリアントに属しているからです。

したがって `hasError` は「このシェーダーは妥当だ」ではなく「インポートがコンパイルした範囲はクリーンだった」と読んでください。再インポートと read を validation という名前のエンドポイントで包んでいないのも同じ理由です。包んだところで死角は同じままで、名前だけがそれを否定することになります。
