...
マテリアルとそのシェーダーが食い違っている箇所を報告します。シェーダーが読む手段を持たないままマテリアルが抱えている値、シェーダーが宣言しているのにマテリアルが値を持っていないプロパティ、そしてマテリアルが有効にしているがシェーダーに居場所のないキーワードの3つです。

報告のみを行います。マテリアルへの書き込みは一切しません。
...
| フィールド | 説明 |
|-----------|------|
| `guid`, `assetPath` | 問い合わせ対象のマテリアル |
| `shader` | 比較対象のシェーダー。[`GET /api/assets/shaders/{guid}`](#get-apiassetsshadersguid) で読めるように識別情報を返します。比較を拒否した場合でも報告します — その状況では「マテリアルが結局どのシェーダーになったか」が答えの大部分だからです。ShaderLab のパースが名前を読む前に失敗した場合、`name` は `null` になります |
| `comparable` | 比較できたかどうか。`false` の場合は3つのリストが `null` になり、`reason` が理由を示します — [比較できない場合](#比較できない場合) を参照 |
| `reason` | `comparable` が `true` なら `null`。そうでなければ `shaderMissing` か `shaderNotRead` |
| `staleProperties[]` | シェーダーの現在の宣言では到達できないまま、マテリアルが抱えている値。宣言が削除された場合と、名前はそのままで型だけが変わった場合の2通りがあります。Unity は `.mat` ファイル内に保持したまま隠すため、他のどのエンドポイントもこれを報告しません — [stale プロパティとは何か](#stale-プロパティとは何か) を参照 |
| `staleProperties[].storage` | マテリアルのシリアライズ上どのマップに入っていたか: `Texture`、`Int`、`Float`、`Color` のいずれか。宣言された型では**ありません**し、そうはなり得ません — 型を示していた宣言そのものが失われているからです。`Float` は `Range` も、`Color` は `Vector` も保持します |
| `staleProperties[].value` | 失われることになる値。storage に応じた表記で返します。`Texture` は参照に加えて、隣に保存されている `scale` と `offset` も返します。3つは一緒に失われるからです |
| `unsetProperties[]` | シェーダーが宣言しているのに、マテリアルがシリアライズされた値を持っていないプロパティ。シェーダーが宣言する `type` と `defaultValue` を伴います。食い違いのないペアでは空です — マテリアル作成時に Unity が宣言済みプロパティすべてにエントリを書くため、ここが埋まるのはマテリアル作成**後に**シェーダーがプロパティを増やした場合です |
| `invalidKeywords[]` | マテリアルが有効にしているキーワードのうち、シェーダーの実効ローカルキーワード空間に存在しないもの。シェーダーが変わってもマテリアル側のキーワードは Unity によって整理されません |

### stale プロパティとは何か

[`GET /api/assets/materials/{guid}`](#get-apiassetsmaterialsguid) はシェーダーが現在宣言しているプロパティとマテリアルの値を返し、[`GET /api/assets/shaders/{guid}`](#get-apiassetsshadersguid) はシェーダーの宣言内容を返します。どちらもシェーダーの**現在の**宣言をたどるため、その宣言から到達できない値は、そのどちらにも現れません。

Unity はその値を捨てません — `.mat` ファイルに残したまま Inspector から隠します。そのためシェーダーを編集すると、古い値は行方の説明もないまま単に消えたように見えます。`staleProperties` はそれに答えるもので、名前だけでなく値も返します。「何かが失われる」ではなく「失われるのはこれだ」が答えになるようにするためです。

値が到達不能になる編集は2通りあり、名前に関係するのは前者だけです。

- **シェーダーが宣言を削除した場合。** プロパティ名の変更はこれが二重に起きます — 旧名の値が取り残され、新名には値がありません。
- **名前はそのままで宣言の型が変わった場合。** 名前は依然として解決できるので、「シェーダーがまだその名前を宣言しているか」だけを見る判定では何も検出できません。6000.0.80f1 で `_Extra` を `Float` から `Color` に宣言し直して実測したところ、`m_Floats` は `_Extra` を 42 のまま保持し、`GetColor("_Extra")` は新しい宣言の既定値を返しました。42 は削除された場合と同じくらい失われています。現在の宣言が読むエントリとそうでないエントリを区別するのが `storage` です。

保存済みエントリがすべて到達不能なプロパティは、`unsetProperties` にも現れます。その場合シェーダーが読める値を1つも持っていないからです。

### `unsetProperties` は「今この瞬間の」シリアライズ状態を表します

新たに到達可能になったプロパティのエントリを Unity は遅延して書き込みます。そのため、誰もマテリアルを触っていないのにプロパティが `unsetProperties` から消えることがあります。6000.0.80f1 で、シェーダーが `_X` を `Float` から `Color` に宣言し直した後、同一の未変更マテリアルに対して2回呼び出して実測しました。1回目は `_X` を `staleProperties`(42 ではなく 7 を保持する到達不能な `Float`)と `unsetProperties` の**両方**に報告しました。`Color` のエントリがまだ存在しなかったためです。2回目は `staleProperties` のみで、その間に Unity がエントリを書き込んでいました。

どちらも問い合わせた時点については正確であり、`staleProperties` は影響を受けません — 取り残された値はどちらでも報告されます。`unsetProperties` は「今この瞬間、このプロパティにシリアライズされた値がない」と読んでください。ペアの恒久的な性質ではありません。

このエンドポイントはそれを削除しません。stale プロパティの削除はマテリアルへの書き込みであり、その結果として捨てられる値は、まさにクライアントが取り戻そうとしているかもしれない値です。読み取りの裏で行うべき操作ではありません。

### `invalidKeywords` が主張すること・しないこと

判定はシェーダーの**実効ローカルキーワード空間**に対して行います。これは Unity が公開しているもので、シェーダー読み取りが `keywords` として返すものと同じです。この空間はシェーダーのソースより広く、`Fallback` や `UsePass` の依存先から来るキーワードや、Unity が自動的に追加するキーワードも含みます。

したがって、ここに挙がったキーワードはそのシェーダーで実際に使用不能です。一方、ここに挙がら**なかった**ことは、シェーダーの作者がそれを宣言した根拠にはなりません。「作者がこのキーワードを宣言したか」は公開 API では一切答えられず、このエンドポイントも答えるふりをしません。

### 比較できない場合

Unity が差し替えたシェーダーはプロパティを1つも宣言しません。それを相手に比較すれば、マテリアルが持つ値すべてを stale として報告することになります — 真実の正反対であり、しかも正しい答えと区別が付きません。そこでこのエンドポイントは答えずに拒否し、`comparable` を `false`、`reason` を設定し、`staleProperties`、`unsetProperties`、`invalidKeywords` をまとめて `null` にします。クライアントが確認すべきものが `comparable` 1つで済むようにするためです。

| `reason` | 原因 |
| --- | --- |
| `shaderMissing` | マテリアルのシェーダーが Unity の内部エラーシェーダーに差し替えられている状態。シェーダーを削除した場合も未割り当ての場合もこうなります。6000.0.80f1 で実測したところ、どちらの経路でも同じ形になりました: `name` は `Hidden/InternalErrorShader`、エラーは記録されておらず、プロパティは0件です |
| `shaderNotRead` | ShaderLab のパースがシェーダー名を読む前に失敗した状態。シェーダー側の構造フィールドが `null` になるのと同じ状態です |

`Fallback` はこれらに**該当しません**。シェーダー自身のサブシェーダーが使用不能で `Fallback` を指定している場合、Unity が差し替えるのはコンパイル済みサブシェーダーであって宣言済みプロパティではないため、プロパティの比較は依然としてマテリアルが指しているシェーダーに対して行われ、結果は妥当です。
...
| `hasError`, `hasWarnings` | Unity がこのシェーダーについて現在キャッシュしている診断にエラー/警告が含まれるかどうか。再インポートが落ち着けばそのインポートの判定になりますが、Unity がバリアントをコンパイルするたびに集合は増えます — [`hasError` は Unity がこれまでにコンパイルしたバリアントの話](#haserror-は-unity-がこれまでにコンパイルしたバリアントの話) を参照。`hasError` は「そのシェーダーが使用不能」を意味**しません**。`isSupported` を参照してください |
| `messages[]` | Unity が現在キャッシュしているコンパイラメッセージ。[診断はキャッシュ済みの内容](#診断はキャッシュ済みの内容) を参照 |
...
- **Unity がそれを受け入れたかどうか。** シェーダーのコンパイルはインポート時に行われ、失敗したシェーダーも見た目はそのままディスク上に残ります。再インポートが落ち着いた後であれば、`hasError` と `messages` は [`POST /api/compile`](compile.ja.md) が C# に対して閉じているのと同じ「編集 → インポート → 診断」のループをシェーダーに対して閉じます。
- **インポートが何を生成したか。** `activeSubshaderIndex` は現在のレンダーパイプラインとプラットフォームによって決まり、どのファイルにも書かれていません。`.shadergraph` に至ってはほぼ全面がそうです。ファイルはノードグラフであり、プロパティ・キーワード・パスはどれもインポート時に生成されます。
...
`messages` は、問い合わせた時点で Unity がそのシェーダーについてキャッシュしている集合であり、その場でのコンパイル結果ではありません。再インポートはこの集合をクリアし、そのインポートがコンパイルした内容で埋め直します。ファイルを編集したら [`POST /api/assets/reimport`](#post-apiassetsreimport) か [`POST /api/editor/refresh`](editor.ja.md#post-apieditorrefresh) で再インポートしてから読み直してください。

編集したアセットを再インポートしてください。`#include` は include 側・シェーダー側のどちらを再インポートしても届き、Unity がまだ知らない `.shader` も `assetPath` で再インポートできます。read の前にインポートが落ち着いたことを確かめたい場合は、再インポートの応答に含まれる `isUpdating` を確認してください。

ディスク上で編集したシェーダーを再インポートせずに read すると、前回のインポートのメッセージが返り、それが古いことを示すものはありません。Unity は「ディスクと相違がある」というシグナルを公開していないため、read から報告しようがないのです。
...
### `hasError` は Unity がこれまでにコンパイルしたバリアントの話

Unity はシェーダーバリアントをインポート時に全部コンパイルするのではなくオンデマンドにコンパイルするため、この集合はひとりでに増えます。6000.0.80f1 での実測では、`multi_compile` のキーワード側の分岐だけが壊れているシェーダーは、再インポート直後は `hasError` が `false`、そのキーワードを有効にしたマテリアルを Scene View がレンダリングすると(再インポートなしで)`true` になり、再インポートすると再び戻りました。

ここから 2 つ言えます。`hasError` がクリーンでも**シェーダーが妥当だという意味ではなく**、壊れたバリアントをまだ誰もコンパイルしていない、という意味です。そして編集していないのに後の read でエラーが現れるのは不具合ではなく、そのバリアントが初めてコンパイラに届いただけです。

### Shader Graph も同じシェーダーアセットとして読める

`.shadergraph` に専用エンドポイントは不要で、ここでの特別扱いもありません。Unity のインポーターがシェーダーを生成してアセットのメインオブジェクトにするため、同じ read がそのまま全項目に答えます。

6000.0.80f1 / Shader Graph 17.0.4 / URP 17.0.4 で、Blackboard プロパティ1件と URP Lit ターゲットを持つグラフを実測した結果:

| フィールド | 報告された内容 |
| --- | --- |
| `name` | `Shader Graphs/<ファイル名>` |
| `properties` | Blackboard プロパティが既定値付きで、Shader Graph が生成するもの(`_QueueOffset`、`_QueueControl`、`unity_Lightmaps` など)と並んで |
| `subshaders[0]` | `renderPipeline` が `UniversalPipeline`、パス10件、それぞれ `lightMode` 付き |
| `keywords` | 60 件 |
| `messages` | `[]` |

注目すべきは `name` です。これはグラフのカテゴリと**ファイル名**を連結したもので、ファイルに書かれているのはカテゴリだけです。つまりグラフを作成・改名した後に、マテリアルが保持し [`POST /api/assets/materials`](#post-apiassetsmaterials) が受け取る文字列を知る手段が、この read になります。`GET /api/assets/shaders?name=` でも到達できます。

これによって `com.unity.shadergraph` が依存関係になることはありません。パッケージは Shader Graph の型を一切参照しておらず、Shader Graph のないプロジェクトでも `GET /api/help` は同じように応答します。

### 2 つのログと、後者が必要な理由

`hasError` と `messages` は**シェーダーコンパイラ**のものです。`hasImportError` と `importMessages` は**アセットインポーター**のものです。統合せずに並置しているのは、シェーダーが片方だけで失敗しうるからです。

コンパイラは Unity が最終的に手にしたシェーダーしか見ていないため、「Unity が手にしたシェーダーが間違っている」とは言えません。そして生成シェーダーでは、それがまさに起こる失敗です。インポーターがビルドできなかった Shader Graph は、そのグラフが本来名乗るはずだった名前を付けた Shader Graph 自身のエラーシェーダーに置き換えられます。この代替シェーダーはクリーンにコンパイルされるので、インポートがどれほど失敗していても `hasError` は `false`、`messages` は空になります。その半面を語れるのはインポーターのログだけであり、だから併せて報告します。

**ただし、そうした失敗がすべて報告されるという意味ではありません。** インポーターのログが運ぶ範囲には境界があり、下の表がその境界です。`hasImportError` がクリーンであることをインポート成功と見なす前に読んでください。

手書きの `.shader` では逆向きです。6000.0.80f1 での実測では、ShaderLab のパースエラーは `messages` を埋め、`importMessages` は空のままでした。`ShaderImporter` はインポートログを使わないからです。

`null` と `[]` は別の答えです。`[]` は「アセットにインポーターがあり、何も記録しなかった」、`null` は「問い合わせるインポーターがない」で、Unity 内蔵シェーダーがこれにあたります(全内蔵アセットが共有するリソースコンテナ経由で到達するため、プロジェクト内のどのインポーターもそれを所有していません)。

**インポーターのログが運ぶもの・運ばないもの。** Shader Graph の失敗すべてがここに届くわけではありません。6000.0.80f1 / Shader Graph 17.0.4 にて:

| 失敗の種類 | エンドポイントの報告 | |
| --- | --- | --- |
| ファイルがパースできない | `400`、`hasImportError` が `true`、`importMessages` にインポーターの例外 | 実測 |
| グラフはパースできるがビルドできない | **何も出ない** — `hasError` は `false`、`hasImportError` も `false`、`importMessages` は `[]`、無名パス1件、プロパティなし | 実測 |
| ターゲットが解決できない | どちらのログにも**何も出ない**。サブシェーダーは黙って落ち、Shader Graph はインポートコンテキストではなく `Debug.LogError` で Console に書きます | 実測 |
| グラフはビルドされるがノード/コンパイラエラーがある | `importMessages` に出ると見込まれます(Shader Graph が Console ではなくインポートコンテキスト経由で記録するため) | **未実測** — Shader Graph のソースから読み取ったもので、これを再現するフィクスチャは見つかっていません |

ターゲットが解決できない場合は [`GET /api/editor/logs`](editor.ja.md#get-apieditorlogs) にメッセージがあります(アセットパスを前置した散文として)。グラフがパースできてビルドできない場合はメッセージ自体がどこにもありません — 6000.0.80f1 での実測では、そのインポートは Console にも何も書きませんでした。したがって `hasError` と `hasImportError` がそろってクリーンでも、インポート成功の証明にはなりません。確実を期すクライアントは、あるはずのものを直接確認してください。パイプラインなら `subshaders[].renderPipeline`、Blackboard に置いた名前なら `properties` です。
