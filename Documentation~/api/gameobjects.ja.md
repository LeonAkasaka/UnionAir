...
配列、ネストされたジェネリック型、`m_Script` は、このエンドポイントが書き込めないプロパティに含まれます。これらを送ることは無視される操作ではなくエラーです。これは配列そのものだけでなく配列の一部にも及びます。`m_Materials.Array.data[0]` や `m_Materials.Array.size` は `m_Materials` と同じ理由で拒否されます。
