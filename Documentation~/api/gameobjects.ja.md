...
### `worldTransform` はオブジェクトが実際にどこにあるかを答えます

`transform` は親からの相対値です。これは `PATCH /api/gameobjects` が書き込む値であり、シーンルート配下のオブジェクトについては、ワールド上のどこにあるかを示しません。`worldTransform` はその隣に常に報告され、リクエスト側で要求する必要はありません。
...
`lossyScale` が Unity の名前をそのまま使っているのは、それが Unity の値だからです。親チェーン全体が生み出すスケールであり、書き戻すことはできません。`PATCH /api/gameobjects` が書き込むのはローカル値である `transform.scale` です。

`right`、`up`、`forward` は、そのオブジェクト自身の軸をワールド空間の単位ベクトルで表したものです。回転値だけではこれらが答える問いに答えられないため報告しています。上の例は人型キャラクターの頭部ボーンで、ローカルの `+X` はワールド空間では**下**を向いています。`right` はそれを直接示しますが、`(359.43, 267.51, 277.86)` から同じことを知るには、読み手が Unity のクォータニオンから基底ベクトルへの変換をやり直す必要があります。ボーンに子オブジェクト(小物やアクセサリ)を配置する作業は、どのローカル方向が上なのかを知ることに尽きます。

`GET /api/scene/hierarchy` はこのフィールドを報告しません。あちらはシーン全体を走査し、上限も500オブジェクトです。1オブジェクトずつ必要になる値のために、ノードごとに6つのベクトルを全呼び出し元が負担することになります。

`PATCH /api/gameobjects` はローカル空間専用のままです。子オブジェクトの transform はローカル空間で記述するため、配置に必要なのは親のワールド座標系を知ることです。ワールド空間での書き込みは別のリクエスト形式であり、ここには含まれません。

### `bounds` はレンダラーのワールド空間のボックスです

`Renderer` コンポーネントは `properties` の隣に `bounds` を報告します。
...
これは `Renderer.bounds`、すなわち**ワールド**空間の軸平行バウンディングボックスであり、オブジェクトが動けば一緒に動きます。`center` と `extents` は Unity 自身の2つのフィールドです(サイズは `2 × extents`、min/max は `center ∓ extents`)。`properties` にあるシリアライズ済みの `m_AABB` は**ローカル**空間のバウンドであり、別の値です。

`Renderer` でないコンポーネントではフィールドごと省略されます。GameObject とその子をまとめたボックスはありません。`POST /api/previews/render` はカメラのフレーミングのために内部で算出していますが、報告はしていません。
