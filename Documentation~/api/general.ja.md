...
アセット参照は、そのオブジェクトのローカルファイル ID を10進文字列で表した `localIdentifier` を持ちます。
...
`assetGuid` と `assetPath` が指すのは*ファイル*です。モデルファイルは多数のメッシュを、スプライトシートは多数のスプライトを、`Library/unity default resources` はビルトインオブジェクト全体を含むため、ファイルと型の組み合わせでは1つのオブジェクトを特定できません。`localIdentifier` はそれを特定します。読み取りはすべてのアセット参照でこれを報告します(単一オブジェクトのアセットへの参照も含みます)。レスポンスの形が参照先によって変わらないようにするためです。

ローカルファイル ID は64ビットであり JSON の数値では表現しきれないため、文字列です。

書き込みでは**省略可能**で、解決は次の3通りです。

| リクエスト | 動作 |
|---|---|
| `localIdentifier` あり | そのオブジェクトを解決します。読み取り結果をそのまま送り返す場合はこれになります |
| `localIdentifier` なし、かつ要求された型のオブジェクトがパス上に1つ | 解決します。大半の参照(マテリアル、テクスチャ、プレハブ)はこれです |
| `localIdentifier` なし、かつ複数存在 | `400`。パス上の候補数を示します |

3番目のケースは、以前は Unity が返した任意のオブジェクトを割り当てたうえで `200` を返しており、クライアントにはどれが割り当てられたか知る手段がありませんでした。

ファイル内のサブアセットとその識別子は [`GET /api/assets/{guid}`](assets.ja.md#get-apiassetsguid) が列挙します。ビルトインリソースは例外で、`localIdentifier` による参照と書き込みは可能ですが、プロジェクトアセットのサブアセットではないため列挙されません。取得するには既存の参照を読み取ってください。

### フィールドの型

参照の各フィールド(`type`、`value`、`scenePath`、`assetGuid`、`assetPath`、`assetType`、`localIdentifier`)は JSON 文字列であるか、省略と同じ意味になる `null` である必要があります。数値、真偽値、配列、オブジェクトを渡した場合、文字列に変換されることはなく、フィールド名を示す `400` を返します。`{"assetGuid": 5}` は GUID `"5"` ではなく、型が誤った値です。
