...
- **Whether Unity accepted it.** Shader compilation happens at import, and a shader that failed still sits on disk looking exactly as it did. Read after a reimport, `hasError` and `messages` close the edit-import-diagnose loop the same way [`POST /api/compile`](compile.md) closes it for C#.
...
A `400` says which of the two it was, and carries `hasImportError`, `hasImportWarnings` and `importMessages` beside `error` whenever the asset has an importer — the log is reported either way, and only the sentence in front of it changes. An asset that is simply not a shader answers `Asset is not a Shader: <path>`; a shader asset whose import failed outright — a `.shadergraph` whose JSON does not parse, for instance — answers `Shader asset failed to import: <path>`, so the edit-import-diagnose loop terminates there too instead of dead-ending.

The second wording needs an import error **and** an extension that produces a shader, which is `.shader` or `.shadergraph`. Every importer writes to the same log, so an import error alone does not make the asset a shader: a `.png` Unity could not read fails its import and types as a `DefaultAsset` exactly like a broken graph, and calling that a failed shader would hand a client that passed the wrong GUID a texture importer's error to act on. `.shadersubgraph` is deliberately not in the set — a Sub Graph's main asset is never a `Shader`, so one that imports cleanly already answers `Asset is not a Shader`, and it should not tell a different story when it fails.
