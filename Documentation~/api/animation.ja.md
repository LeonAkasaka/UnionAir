...
### 組の書き込みが失敗する 2 つの形と、それぞれの報告

どちらも組の書き込みの帰結です。どちらもかつては `errors` が空の `200` を返していましたが、現在は報告されます。6000.0.80f1 での実測です。

**成分名が欠けている／その組の成分でない場合、指定したキーが失われます — `400`。** 組を選ぶのは接頭辞だけで、末尾の成分名はどの成分にカーブを載せるかを選びます。したがって成分名がその組のどれでもないとき、キーの置き場所がなく、組は空のまま作られます。
...
そのエントリは `errors` で報告され、代わりに指定できた成分名が併記されます。判定はクリップの最終状態ではなく、**そのエントリ単独で何が保存されるか**に対して行われます。組に既存のカーブがある場合、この書き込みは**完全な no-op** になり、既にあったキーはそのまま残り、送ったキーだけが同じように捨てられるためです。成分名は正確に指定してください。

**クォータニオンで回転を書くときは、4 成分すべてが必要です — `warnings` に 1 件。** `localRotation.y` の 1 エントリでも 4 バインディングは作られますが、`w` が `0` で埋まります。クォータニオン `(0, y, 0, 0)` は `y` の値によらず半回転に正規化されます。
...
このエンドポイントは、リクエストが触れた `m_LocalRotation` の組について 4 本のカーブをキーの打たれている時刻で評価し、そこでクォータニオンが単位長でなければ警告します。検出できるのは `w` の埋めだけではありません。組の書き込みは全成分を組のキー時刻の和集合に再サンプリングするため、`0` と `2` にだけキーを打った成分は `1` で**補間値**を受け取ります。`y` が `0.7071` のとき `w` は `0.5`、長さ `0.866`、回転は意図した 90° ではなく 109.5° になります。

キー時刻だけを見るのは、正しく書かれたクォータニオンもキーの**間**では単位長にならないからです。Unity は 4 成分を補間し、適用時に正規化します。

**error ではなく warning である理由。** 書き込み自体はカーブを保存しており、また呼び出し側が複数リクエストにわたってクォータニオンを埋めていくのは正当な使い方だからです(後続の書き込みは対象の成分だけを置き換え、既にカーブを持つ成分はそのまま残します)。その場合は最後の 1 回を除くすべてで警告が出ますが、これは正確です — 最後が届くまで、クリップは実際に誤って再生される回転を保持しています。完成したクォータニオンは、1 リクエストで送っても 4 リクエストに分けても警告されません。
...
| `errors` | 拒否されたエントリ、キーを 1 つも保存できなかったエントリ、および生成されるはずでクリップに存在しなかったバインディング |
| `warnings` | 書き込みは成立したが、結果が意図したものでない可能性が高いもの。現時点では上記の非単位クォータニオン |

どちらの配列も常に存在し、報告すべきものがなければ空です。
...
キーを 1 つも保存できなかったエントリでも、生成されたバインディングは `added` と `bindings` に載ります。それらは実際にクリップ上にあり `DELETE .../curves` が受け付けるためです。書き込みが役目を果たさなかったことを示すのは `errors` の項目です。

### エラー

| ステータス | 原因 |
|--------|-------|
| 400 | カーブの必須フィールドの欠落、未知の型、または指示どおりに保存できたエントリが 1 つもない |
| 404 | 指定 GUID のアセットが見つからない |
| 403 | Asset Write カテゴリが無効 |

ステータスを決めるのは「クリップにバインディングが増えたか」ではなく「**いずれかの**エントリがキーを保存できたか」です。成分名が組のどれでもない書き込みは組そのものは作るので、バインディング数で判定するとこの Issue の発端となったリクエストに `200` を返してしまいます。正常なエントリと不正なエントリが混在するリクエストは従来どおり `200` を返し、不正な方を `errors` で報告します。
...
コントローラーの一部をアドレス指定するアニメーション系エンドポイントはすべて `layerIndex` を取り、既定値は `0` です。ボディでもクエリパラメータ(`?layerIndex=1`)でも送れ、両方ある場合はボディが優先されます。値が存在していて整数でない場合は、ベースレイヤーへ黙って落ちるのではなく `400` になります。

`stateMachinePath` にクエリ形式はありません。配列であり、クエリ文字列に配列を曖昧さなく載せる方法がないためです。
