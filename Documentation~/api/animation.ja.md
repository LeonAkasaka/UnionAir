...
`*Parameter` とその `*Active` フラグは 1 つの決定として扱います。コントローラーに無い名前は拒否し、名前もフラグも書き込みません。存在しないパラメータでオーバーライドを有効にすると、再生できないステートが残るためです。名前が `""` の場合はオーバーライドの解除として扱い、参照の確認は行いません。
