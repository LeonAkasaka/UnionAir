...
A clip inside an `.fbx` is generated by a `ModelImporter`, which owns its settings. `AnimationUtility.SetAnimationClipSettings` on one mutates an in-memory object that the next reimport discards, so **every write endpoint refuses an imported clip with `409`** — including `POST` and `DELETE .../curves`, which used to accept the write and lose it silently. Changing an imported clip means changing the importer, which is [`GET|PATCH /api/assets/model-importer/{guid}`](assets.md#get-apiassetsmodel-importerguid). The settings this endpoint refuses to write — loop, cycle offset, the root motion bake settings, the additive reference pose, the events — are fields of one clip definition there. The two directions do not spell it the same way: the read reports the definitions under `settings.clips.definitions`, and the write takes them as the top-level `clips` array, which replaces the stored set wholesale.
