...
The array is positional: index *i* names the shape that `properties.m_BlendShapeWeights[i]` drives, which is how a weight is written. Names are not unique — Unity permits two shapes on one mesh to carry the same name — so the index is the identity and the name is the description. The two arrays can also differ in length: the names are read from the mesh as it is imported now, the weights from what was serialized on the component, and reimporting a mesh with fewer shapes leaves the serialized array as it was until Unity rewrites it. Neither array is corrected to match the other; index defensively.
