...
Editor state, logs, selection, cameras, isolated object/animation previews, capture, refresh, and menu items:
...
`GET /api/assets` · `GET /api/assets/{guid}` · `GET /api/search/asset-refs` · `GET /api/assets/dependents` · `POST /api/assets/prefabs` (+ `apply`, `revert`) · `GET /api/assets/materials/{guid}` · `POST|PATCH /api/assets/materials` · `DELETE /api/assets/{guid}` · `POST /api/assets/move` · `POST /api/assets/open` · `POST /api/assets/reimport` · ScriptableObject CRUD (`/api/assets/scriptableobjects`) · `PATCH /api/assets/texture-importer/{guid}` · `GET|PATCH /api/assets/audio-importer/{guid}` · ModelImporter core, material/remap, rig/Avatar, and imported-clip settings through `GET|PATCH /api/assets/model-importer/{guid}` and `POST /api/assets/model-importer/{guid}/preflight`
